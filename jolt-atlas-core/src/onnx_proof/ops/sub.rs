...
impl_standard_sumcheck_handler!(Sub, SubParams, SubProver, SubVerifier);
...
impl<F: JoltField> SumcheckInstanceParams<F> for SubParams<F> {
    ...
    fn input_claim(&self, accumulator: &dyn OpeningAccumulator<F>) -> F {
        let (_, sub_claim) = accumulator.get_virtual_polynomial_opening(
...
mod tests {
    ...
    fn test_sub() {
        ...
        let model = model::test::sub_model(&mut rng, T);
        ...
        let sub_claim = MultilinearPolynomial::from(output.clone()).evaluate(&r_node_output);
        ...
        let params: SubParams<Fr> =
            SubParams::new(computation_node.clone(), &prover_opening_accumulator);
        ...
        let verifier_sumcheck =
            SubVerifier::new(computation_node.clone(), &verifier_opening_accumulator);
