...
use crate::compat::utils::unsafe_zero_slice;
use jolt_core::{
    field::JoltField,
    poly::{
...
impl<'a, F: JoltField> ReadRafSumcheck<F> {
    ...
    pub fn new_prover(
        ...
        let mut ps = ReadRafProverState::new(sm, eq_r_cycle);
        ps.init_phase(0);
        let r_cycle = sm
...
impl<'a, F: JoltField> ReadRafProverState<F> {
    fn new<ProofTranscript: Transcript, PCS: CommitmentScheme<Field = F>>(
        ...
        let (lookup_indices_uninterleave, lookup_indices_identity): (Vec<_>, Vec<_>) =
            lookup_indices
                ...
                .cloned()
                .enumerate()
                .zip(bytecode.par_iter())
                .partition_map(|((idx, item), instr)| {
        ...
        let (is_interleaved_operands, lookup_tables): (Vec<_>, Vec<_>) = trace
            ...
            .map(|(cycle, instr)| {
                (
                    instr.circuit_flags().is_interleaved_operands(),
                    cycle.lookup_table(),
                )
            })
            .collect();
        ...
        ReadRafProverState {
            ...
            suffix_polys,
            v: ExpandingTable::new(M, BindingOrder::HighToLow),
...
impl<F: JoltField> SumcheckInstance<F> for ReadRafSumcheck<F> {
    ...
    fn expected_output_claim(
        ...
        // Convert challenges to field elements for polynomial evaluations
        let r_address_prime_field: Vec<F> = r_address_prime.iter().map(|&c| c.into()).collect();
        let left_operand_eval =
            OperandPolynomial::new(LOG_K, OperandSide::Left).evaluate(&r_address_prime_field);
        let right_operand_eval =
            OperandPolynomial::new(LOG_K, OperandSide::Right).evaluate(&r_address_prime_field);
        let identity_poly_eval = IdentityPolynomial::new(LOG_K).evaluate(&r_address_prime_field);
...
impl<F: JoltField> ReadRafProverState<F> {
    ...
    fn init_phase(&mut self, phase: usize) {
        ...
        rayon::scope(|s| {
                            ...
                            .for_each(|(suffix, poly)| {
                                ...
                                for (j, k) in lookup_indices.iter() {
                                    ...
                                    if t != 0 {
                                        ...
                                        poly.Z[usize::from(prefix_bits) % M] += u.mul_u64(t as u64);
                                    }
                                }
                            });
                    });
            ...
            s.spawn(|_| {
                let (indices, lookup_bits): (Vec<usize>, Vec<LookupBits>) =
                    self.lookup_indices_uninterleave.iter().cloned().unzip();
                self.right_operand_ps
                    .init_Q(&self.u_evals, &indices, &lookup_bits)
            });
            s.spawn(|_| {
                let (indices, lookup_bits): (Vec<usize>, Vec<LookupBits>) =
                    self.lookup_indices_uninterleave.iter().cloned().unzip();
            ...
            s.spawn(|_| {
                let (indices, lookup_bits): (Vec<usize>, Vec<LookupBits>) =
                    self.lookup_indices_identity.iter().cloned().unzip();
                self.identity_ps
                    .init_Q(&self.u_evals, &indices, &lookup_bits)
            });
        });
