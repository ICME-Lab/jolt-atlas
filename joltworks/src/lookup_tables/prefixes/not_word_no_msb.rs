...
pub enum NotWordNoMsbPrefix<const XLEN: usize> {}

impl<const XLEN: usize, F: JoltField> SparseDensePrefix<F> for NotWordNoMsbPrefix<XLEN> {
    fn prefix_mle<C>(
    ...
    {
        let mut word = checkpoints[Prefixes::NotWordNoMsb].unwrap_or(F::one());
        if j >= XLEN {
