    ...
    F: JoltField,
{
    ...
    fn compute_prefix_suffix_prover_message(&self, _round: usize, previous_claim: F) -> UniPoly<F> {
        UniPoly::from_evals_and_hint(previous_claim, &self.prover_msg_raf())
    }
    ...
    F: JoltField,
{
    ...
    fn phases(&self) -> usize {
        ...
        // Default to 8 phases, but allow overriding for smaller log_K values to avoid very small phase sizes
        if log_K <= 4 {
            1
        } else {
            log_K / 4
