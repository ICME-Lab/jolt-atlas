...
//! via a sumcheck over: Sum_x eq(tau,x) * [Az(x)*Bz(x) - u*Cz(x) - E(x)] = 0
//!
...
/// eq(tau,x) * Az(x) * Bz(x) is degree 3 in each variable.
...
impl<F: JoltField> SumcheckInstanceParams<F> for BlindFoldSpartanParams<F> {
    ...
    fn input_claim(&self, _accumulator: &dyn OpeningAccumulator<F>) -> F {
        // Spartan starts with claim = 0 (we're proving Sum eq(tau,x)*[...] = 0)
        F::zero()
    }
    ...
    #[cfg(feature = "zk")]
    fn input_claim_constraint(&self) -> super::InputClaimConstraint {
        // No input constraint - Spartan starts at 0
        super::InputClaimConstraint::default()
    }
    ...
    #[cfg(feature = "zk")]
    fn output_claim_constraint(&self) -> Option<super::OutputClaimConstraint> {
...
///
/// Proves: Sum_x eq(tau,x) * [Az(x)*Bz(x) - u*Cz(x) - E(x)] = 0
...
impl<F: JoltField, T: Transcript> SumcheckInstanceProver<F, T> for BlindFoldSpartanProver<'_, F> {
    ...
    fn cache_openings(
        ...
        // Opening proofs for W and E contributions are handled by BlindFold
        // after verifying the final claim.
    }
...
///
/// Proves: Sum_j L_w(j) * W(j) = claim_inner
/// where L_w(j) = ra*A'[ws+j] + rb*B'[ws+j] + rc*C'[ws+j]
/// and A'[col] = Sum_row A[row,col]*eq(rx,row).
///
...
impl<F: JoltField> BlindFoldInnerSumcheckProver<F> {
    ...
    /// Compute the degree-2 round polynomial
