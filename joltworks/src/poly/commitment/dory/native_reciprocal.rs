//! Exact floor reciprocal of a hidden positive integer for normalization.
//! With A=2^(2*scale), prove x*q+r=A and x-r-1=gap with nonnegative integer
//! columns. These identities require x>0 and 0<=r<x. Supported scales are 0..=15.
...
impl ReciprocalWitness {
    pub fn new(input: &[i32], scale: u8) -> Result<Self, ProofVerifyError> {
        ...
        for x in input {
            ...
            let q = a / x;
            let r = a % x;
            for (j, v) in [x, q, r, x - r - 1].iter().enumerate() {
                columns[j].push(*v);
            }
        }
        let output = columns[1]
            ...
            .collect();
        let mut polynomials: BTreeMap<_, _> = columns
            .iter()
            .enumerate()
...
pub(super) struct ReciprocalRegistration {
...
impl ReciprocalRegistration {
    ...
    fn params(&self, r: &[Fr], t: &mut Blake2bTranscript) -> ReciprocalParams {
        let mut params = ReciprocalParams::new(r, self.scale, t);
    ...
    pub fn provers(
        ...
        let arithmetic = ReciprocalProver {
    ...
    pub fn verifiers(
        ...
        let mut result: Vec<Box<dyn SumcheckInstanceVerifier<Fr, Blake2bTranscript>>> =
            vec![Box::new(ReciprocalVerifier(self.params(&r, t)))];
        for range in self.ranges() {
...
impl ReciprocalParams {
    fn new(r: &[Fr], scale: u8, t: &mut Blake2bTranscript) -> Self {
        Self {
            ...
            a: Fr::from(1u64 << (2 * scale)),
            gamma: std::array::from_fn(|_| t.challenge_scalar()),
        }
    }
    fn evaluate(&self, v: &[Fr; N]) -> Fr {
        let [x, q, r, gap] = *v;
        self.gamma[0] * (x * q + r - self.a) + self.gamma[1] * (x - r - Fr::from(1u64) - gap)
    }
    fn coefficients(&self) -> Vec<Fr> {
        vec![
            ...
            self.gamma[1],
            -self.gamma[1],
            -self.gamma[0] * self.a - self.gamma[1],
        ]
    }
}

impl SumcheckInstanceParams<Fr> for ReciprocalParams {
...
struct ReciprocalProver {
    #[allocative(skip)]
    params: ReciprocalParams,
    values: [MultilinearPolynomial<Fr>; N],
    eq: MultilinearPolynomial<Fr>,
}
impl SumcheckInstanceProver<Fr, Blake2bTranscript> for ReciprocalProver {
...
struct ReciprocalVerifier(ReciprocalParams);
impl SumcheckInstanceVerifier<Fr, Blake2bTranscript> for ReciprocalVerifier {
    fn get_params(&self) -> &dyn SumcheckInstanceParams<Fr> {
        &self.0
    }
