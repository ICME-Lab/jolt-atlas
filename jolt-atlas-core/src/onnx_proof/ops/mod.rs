...
// Note: The shared Params boilerplate (struct, new(), SumcheckInstanceParams impl) is now
// generated by impl_standard_params! for Add, Sub, Mul, Square, Cube. Operators with bespoke
// polynomial structures (Div, Iff, Rsqrt, Tanh, Softmax) keep their own hand-written impls.

/// Shared plumbing for the clamped Erf/Sigmoid/Tanh variants.
pub mod activation_clamped;
...
/// Macro to generate the standard `Params` struct, constructor, and [`SumcheckInstanceParams`]
/// implementation for arithmetic operators (Add, Sub, Mul, Square, Cube, etc.).
///
/// All these operators share identical fields and protocol structure; only the sumcheck
/// degree varies. This macro eliminates ~60 lines of repeated boilerplate per operator.
///
/// # Requirements
///
...
///
/// ```rust,ignore
/// impl_standard_params!(AddParams, 2); // generates a degree-2 sumcheck params struct
/// ```
#[macro_export]
macro_rules! impl_standard_params {
    ($params_ty:ident, $degree:expr) => {
        /// Shared parameter block for a standard element-wise sumcheck proof.
        ///
        /// Stores the output's opening point and computation node needed to configure
        ...
        impl<F: JoltField> SumcheckInstanceParams<F> for $params_ty<F> {
