...
where
    ...
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    /// Returns `(left_eval, right_eval)` at `r_address` using the
    /// signed operand polynomials.
    pub fn raf_operand_evals(&self, r_address: &[F]) -> (F, F) {
        (
    ...
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    ...
    pub fn new(params: ReadRafSumcheckParams<F, LUT>, lookup_indices: Vec<LookupBits>) -> Self {
        ...
        drop(_guard);
        Self::new_inner(params, lookup_indices, left_operand_ps, right_operand_ps)
    }
    ...
    fn init_phase(&mut self, phase: usize) {
        ...
        PrefixSuffixDecomposition::init_Q_dual(left_ps, right_ps, u_evals, &self.lookup_indices);
    ...
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    ...
    fn expected_output_claim(
        ...
        let (left_operand_eval, right_operand_eval) =
