...
impl Args {
    fn parse() -> Result<Self, Box<dyn Error>> {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap()
            .to_path_buf();
        let mut model = root.join("atlas-onnx-tracer/models/qwen/model.safetensors");
        let mut tokenizer = root.join("atlas-onnx-tracer/models/qwen/tokenizer.json");
...
fn print_help() {
    println!(
        "qwen2-awy\n\
         \n\
         Run full f32 Qwen2 with optional MatMul A/W/Y quantize-dequantize steps.\n\
         \n\
         Options:\n\
           ...
           --temperature T       sampling temperature; enables --sample\n\
           --top-p P             nucleus sampling cutoff; enables --sample\n\
           --top-k K             keep only top K candidates before top-p; enables --sample\n\
...
fn load_layer(st: &SafeTensors<'_>, layer: usize) -> Result<LayerWeights, Box<dyn Error>> {
    ...
    Ok(LayerWeights {
        ...
        wq: linear_f32(st, &format!("{p}.self_attn.q_proj.weight"), HIDDEN, HIDDEN)?,
        bq: vec_f32(st, &format!("{p}.self_attn.q_proj.bias"), &[HIDDEN])?,
        wk: linear_f32(
            st,
        ...
        wo: linear_f32(st, &format!("{p}.self_attn.o_proj.weight"), HIDDEN, HIDDEN)?,
        wg: linear_f32(
            st,
...
fn generate_greedy_cached(
    ...
    let mut logits = {
        let h = rms_norm(&last_hidden, &norm, 1, HIDDEN);
    ...
    for _ in 0..max_new_tokens {
        ...
        let h = rms_norm(&hidden, &norm, 1, HIDDEN);
...
fn decode_one_cached(
    ...
    let mut x = embed_one_from_safetensors(st, id)?;
...
fn layer_forward(
    ...
    let a = matmul_awq(&c, &w.wo, seq, HIDDEN, HIDDEN, MatmulSite::O, cfg, report);
    let h = add(x, &a);
...
fn layer_forward_with_stats(
    ...
    let mut q = matmul_awq(&n1, &w.wq, seq, HIDDEN, HIDDEN, MatmulSite::Q, cfg, report);
    add_rows(&mut q, &w.bq);
    let mut k = matmul_awq(
        &n1,
        &w.wk,
        seq,
        HIDDEN,
...
fn layer_decode_cached(
    ...
    let n1 = rms_norm(x, &w.ln1, 1, HIDDEN);
    let mut q = matmul_cached_awq_scaled(
        &n1,
        &w.wq,
        qw.map(|q| q.wq.as_slice()),
    ...
    add_rows(&mut v, &w.bv);
    let q = rope_one(&q, &r.rq, pos, HEADS);
    let k = rope_one(&k, &r.rk, pos, KV_HEADS);
    cache.k.extend_from_slice(&k);
    cache.v.extend_from_slice(&v);
    let context_len = cache.v.len() / (KV_HEADS * HEAD_DIM);
    let c = attn_v_decode(&q, &cache.k, &cache.v, context_len);
    let a = matmul_cached_awq(
        &c,
        &w.wo,
        qw.map(|q| q.wo.as_slice()),
    ...
    let h = add(x, &a);
    let n2 = rms_norm(&h, &w.ln2, 1, HIDDEN);
    let g = matmul_cached_awq_scaled(
        &n2,
        &w.wg,
        qw.map(|q| q.wg.as_slice()),
...
fn matmul_awq(
    ...
    if matches!(cfg.quant_mode, QuantMode::Fixed) {
        let y = matmul_awq_fixed_int(a, w, m, k, n, cfg);
        if report.enabled {
...
fn matmul_awq_fixed_int(
    ...
    match (cfg.a_bits.0, cfg.w_bits.0) {
...
fn matmul_cached_awq(
    ...
    let Some(a_frac) = cfg.a_bits.0 else {
        return matmul_awq(a, w, m, k, n, site, cfg, report);
    };
...
fn score_qk(q: &[f32], k: &[f32], seq: usize) -> Vec<f32> {
    ...
    assert_eq!(k.len(), seq * KV_HEADS * HEAD_DIM);
...
fn attn_v(p: &[f32], v: &[f32], seq: usize) -> Vec<f32> {
    ...
    assert_eq!(v.len(), seq * KV_HEADS * HEAD_DIM);
    let mut y = vec![0.0; seq * HIDDEN];
...
fn attn_v_decode(q: &[f32], k: &[f32], v: &[f32], context_len: usize) -> Vec<f32> {
    ...
    assert_eq!(k.len(), context_len * KV_HEADS * HEAD_DIM);
    assert_eq!(v.len(), context_len * KV_HEADS * HEAD_DIM);
...
impl Report {
    ...
    fn print(&self) {
        ...
        if !self.int_width_sites.is_empty() {
            ...
            println!(
                "{:<12} {:>6} {:>10} {:>10} {:>10}",
                "site", "count", "A_QX.0b", "W_QX.0b", "Y_QX.0b"
            );
            for (&site, rows) in &self.int_width_sites {
                ...
                println!("{site:<12} {:>6} {a:>10} {w:>10} {y:>10}", rows.len());
            }
        }
    }
}
