    ...
    F: JoltField,
{
    ...
    fn bytecode_to_val(program_bytecode: &[ONNXInstr], gamma: &F) -> Vec<F> {
        const DEGREE: usize = 7 + MAX_TENSOR_SIZE;
        ...
        program_bytecode
            ...
            .map(|instr| {
                ...
                linear_combination +=
                    (instr.ts3.unwrap_or_default() as u64).field_mul(gamma_pows[4]);
                linear_combination +=
                    (instr.td.unwrap_or_default() as u64).field_mul(gamma_pows[5]);
                linear_combination +=
                    (instr.active_output_elements as u64).field_mul(gamma_pows[6]);
                (0..MAX_TENSOR_SIZE)
                    .map(|i| instr.imm()[i].field_mul(gamma_pows[7 + i]))
