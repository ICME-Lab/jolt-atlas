...
use crate::{
    jolt::{
        ...
        precompiles::{
            PrecompileTrait,
            matvecmult::{execution::ExecutionSumcheck, read_checking::ReadCheckingABCSumcheck},
        },
...
impl PrecompilePreprocessingTrait for MatVecPreprocessing {}
...
impl MatVecPreprocessing {
    ...
    pub fn new(
        ...
        let a_instr = PreprocessingHelper::get_operand_instruction(td_lookup, instr.ts1, "MatMult");
        ...
        let (k_padded, n_padded) = PreprocessingHelper::calculate_padded_dimensions(k, n);

        // Collect and process addresses
        let a = PreprocessingHelper::collect_and_resize_vector(
            a_instr,
            bytecode_preprocessing,
            k_padded,
        );
        let b = PreprocessingHelper::collect_and_pad_matrix(
            b_instr,
            bytecode_preprocessing,
            k,
            k_padded,
        ...
        let c =
            PreprocessingHelper::collect_and_resize_vector(instr, bytecode_preprocessing, n_padded);

        MatVecPreprocessing { a, b, c }
    }
}
...
impl PrecompileTrait for MatVecPrecompile {
    ...
    >(
        ...
        let matvec_pp = &pp.shared.precompiles.mat_vec[self.index];
