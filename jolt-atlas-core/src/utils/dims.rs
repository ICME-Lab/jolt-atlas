...
fn extract_mk_kn_mn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let m = if computation_node.raw_or_padded_output_dims().len() == 3 {
        computation_node.raw_or_padded_output_dims()[1]
    } else {
        computation_node.raw_or_padded_output_dims()[0]
    };
    let k = b_node.raw_or_padded_output_dims()[0];
    let n = b_node.raw_or_padded_output_dims()[1];
...
fn extract_k_nk_n_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let _a_node = &model[a_idx];
    let b_node = &model[b_idx];
    let n = b_node.raw_or_padded_output_dims()[0];
    let k = b_node.raw_or_padded_output_dims()[1];
...
fn extract_m_an_a1nm_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let b_node = &model[b_idx];
    let m = a_node.raw_or_padded_output_dims()[0];
    let a = b_node.raw_or_padded_output_dims()[0];
    let n = b_node.raw_or_padded_output_dims()[1];

    EinsumDims::new(
        vec![m],
        vec![a, n],
        computation_node.raw_or_padded_output_dims(),
    )
}
...
fn extract_mbk_nbk_bmn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let b_node = &model[b_idx];
    let m = a_node.raw_or_padded_output_dims()[0];
    let b = a_node.raw_or_padded_output_dims()[1];
    let k = a_node.raw_or_padded_output_dims()[2];
    let n = b_node.raw_or_padded_output_dims()[0];
...
fn extract_bmk_kbn_mbn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let _a_node = &model[a_idx];
    let b_node = &model[b_idx];
    let m = computation_node.raw_or_padded_output_dims()[0];
    let b = computation_node.raw_or_padded_output_dims()[1];
    let n = computation_node.raw_or_padded_output_dims()[2];
    let k = b_node.raw_or_padded_output_dims()[0];
...
fn extract_mbk_bkn_amn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let b = a_node.raw_or_padded_output_dims()[1];
    let k = a_node.raw_or_padded_output_dims()[2];
    let n = b_node.raw_or_padded_output_dims()[2];
...
fn extract_abmk_abnk_abmn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let a_node = &model[a_idx];
    let b_node = &model[b_idx];
    let a = a_node.raw_or_padded_output_dims()[0];
    let b = a_node.raw_or_padded_output_dims()[1];
    let m = a_node.raw_or_padded_output_dims()[2];
    let k = a_node.raw_or_padded_output_dims()[3];
    let n = b_node.raw_or_padded_output_dims()[2];
...
fn extract_acbmk_kcn_cbmn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let b_node = &model[b_idx];
    let a = a_node.raw_or_padded_output_dims()[0];
    let c = a_node.raw_or_padded_output_dims()[1];
    let b = a_node.raw_or_padded_output_dims()[2];
    let m = a_node.raw_or_padded_output_dims()[3];
    let k = a_node.raw_or_padded_output_dims()[4];
    let n = b_node.raw_or_padded_output_dims()[2];
...
fn extract_cbmk_cbkn_amn_dims(computation_node: &ComputationNode, model: &Model) -> EinsumDims {
    ...
    let a_node = &model[a_idx];
    let b_node = &model[b_idx];
    let c = a_node.raw_or_padded_output_dims()[0];
    let b = a_node.raw_or_padded_output_dims()[1];
    let m = a_node.raw_or_padded_output_dims()[2];
    let k = a_node.raw_or_padded_output_dims()[3];
    let n = b_node.raw_or_padded_output_dims()[3];
...
pub fn sum_config(computation_node: &ComputationNode, model: &Model) -> SumConfig {
    ...
    let input_dims = &model[input_idx].raw_or_padded_output_dims();
    let output_dims = &computation_node.raw_or_padded_output_dims();
