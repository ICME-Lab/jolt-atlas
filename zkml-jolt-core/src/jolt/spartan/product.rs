use crate::jolt::{
    ...
    r1cs::inputs::JoltONNXR1CSInputs,
...
pub enum VirtualProductType {
    ...
    Instruction,
    /// td_addr × WriteLookupOutputToTD_flag
...
impl VirtualProductType {
    pub fn get_virtual_polynomial(&self) -> VirtualPolynomial {
        match self {
            ...
            VirtualProductType::WriteLookupOutputToTD => VirtualPolynomial::WriteLookupOutputToTD,
            VirtualProductType::SelectCond => VirtualPolynomial::SelectCond,
            VirtualProductType::SelectRes => VirtualPolynomial::SelectRes,
        }
    }
    pub fn get_factor_polynomials(&self) -> FactorPolynomials {
        match self {
            ...
            VirtualProductType::SelectRes => FactorPolynomials(
    ...
    pub fn get_sumcheck_id(&self) -> SumcheckId {
        match self {
            ...
            VirtualProductType::SelectCond => SumcheckId::SelectCondVirtualization,
            VirtualProductType::SelectRes => SumcheckId::SelectResVirtualization,
        }
    }

    pub fn get_r1cs_inputs(&self) -> (JoltONNXR1CSInputs, JoltONNXR1CSInputs) {
        match self {
    ...
    {
        let (left_input, right_input) = self.get_r1cs_inputs();
        (
            left_input.generate_witness(trace, preprocessing),
            right_input.generate_witness(trace, preprocessing),
        )
    }
}
...
impl<F: JoltField> ProductVirtualizationSumcheck<F> {
    ...
    pub fn new_prover<ProofTranscript: Transcript, PCS: CommitmentScheme<Field = F>>(
        ...
        let (left_input_poly, right_input_poly) = product_type.get_input_polys(trace, pp);
...
impl<F: JoltField> SumcheckInstance<F> for ProductVirtualizationSumcheck<F> {
    ...
    fn cache_openings_verifier(
        ...
        accumulator
            .borrow_mut()
            .append_virtual(left_poly, sumcheck_id, opening_point.clone());
        accumulator
            .borrow_mut()
            .append_virtual(right_poly, sumcheck_id, opening_point);
    }
}
