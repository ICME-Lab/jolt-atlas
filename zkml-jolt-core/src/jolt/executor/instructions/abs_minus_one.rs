use crate::jolt::{
    ...
    lookup_table::{LookupTables, TestTable},
};
use jolt_core::zkvm::instruction::LookupQuery;
use serde::{Deserialize, Serialize};

#[derive(Copy, Clone, Default, Debug, Serialize, Deserialize, PartialEq)]
pub struct TestInstruction<const WORD_SIZE: usize>(pub u64);

impl<const WORD_SIZE: usize> InstructionLookup<WORD_SIZE> for TestInstruction<WORD_SIZE> {
    fn lookup_table(&self) -> Option<LookupTables<WORD_SIZE>> {
        Some(TestTable.into())
    }
}

impl<const WORD_SIZE: usize> LookupQuery<WORD_SIZE> for TestInstruction<WORD_SIZE> {
...
mod test {
    ...
    fn materialize_entry() {
        materialize_entry_test(ONNXOpcode::Test);
    }
}
