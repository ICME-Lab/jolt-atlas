//! Benchmark-only sibling of `softmax_last_axis` that replaces the `sat_diff` complementary-
//! slackness sumcheck with a `ClampBoundedTable`-based lookup
//! (`joltworks::lookup_tables::clamp::SoftmaxSatClampTable`, via `sat_clamp::SoftmaxSatClampOperands`).
//!
//! Reuses `exp_sum`/`max`/`recip_mult`/`exponentiation`/`rc` directly from the original
//! `softmax_last_axis` module (unchanged) — only the saturating-clamp step differs. See
//! `sat_clamp.rs`'s module doc for the relation being proven, and the plan file
//! (`i-want-to-benchmark-whimsical-tiger.md`) for the full design rationale.
//!
//! For more on the overall claim flow, see: https://hackmd.io/@R5TO3fi7TlWQKnY6Ejr2eg/Byjucoqjbx
    ...
    onnx_proof::{
        ...
        ops::{
            softmax_last_axis::{
                ...
                LookupTableData, VerifierLookupTableData,
            },
            OperatorProofTrait,
        },
...
use atlas_onnx_tracer::{
    ...
    ops::{
        softmax::{
            generate_exp_lut_decomposed, softmax_last_axis_decomposed_padded_bound, softmax_z,
            SoftmaxLastAxisTrace,
        },
        SoftmaxLastAxisSatClamp,
    },
    tensor::Tensor,
...
impl<F: JoltField, T: Transcript> OperatorProofTrait<F, T> for SoftmaxLastAxisSatClamp {
    #[tracing::instrument(skip_all, name = "SoftmaxLastAxisSatClamp::prove")]
    fn prove(
        &self,
        node: &ComputationNode,
        ...
        debug_assert_eq!(
            ...
            "SoftmaxLastAxisSatClamp only supports the currently-compiled MODEL_SCALE={MODEL_SCALE}"
        );
        let scale = scale_to_multiplier(self.scale) as i32;
        let softmax_input = prover.trace.operand_tensors(node)[0];
        let trace = softmax_last_axis_decomposed_padded_bound(softmax_input, scale).1;
        SoftmaxLastAxisSatClampProver::new(node, trace, scale).prove(prover)
    }

    #[tracing::instrument(skip_all, name = "SoftmaxLastAxisSatClamp::verify")]
    fn verify(
        &self,
        node: &ComputationNode,
        ...
        let scale = scale_to_multiplier(self.scale) as i32;
        let mut sm = SoftmaxLastAxisSatClampVerifier::new(
            node,
            scale,
...
pub(crate) struct SoftmaxLastAxisSatClampProver {
...
impl SoftmaxLastAxisSatClampProver {
    #[inline]
    fn idx(&self) -> usize {
        self.computation_node.idx
    }

    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::prove", skip_all)]
    ...
    fn prove<F: JoltField, T: Transcript>(
        ...
        // now that sat_diff's interactive binding no longer produces these claims.
...
/// Verifier for the sat-clamp softmax variant. Mirror of [`SoftmaxLastAxisSatClampProver`].
pub(crate) struct SoftmaxLastAxisSatClampVerifier {
...
impl SoftmaxLastAxisSatClampVerifier {
    #[inline]
    pub(crate) fn idx(&self) -> usize {
        self.computation_node.idx
    }

    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::verify", skip_all)]
...
// ---------------------------------------------------------------------------
// SoftmaxLastAxisSatClampProver Methods
// ---------------------------------------------------------------------------
...
impl SoftmaxLastAxisSatClampProver {
    ...
    #[tracing::instrument(
        name = "SoftmaxLastAxisSatClampProver::send_auxiliary_vectors",
        skip_all
    )]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::cache_exp_sum", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::stage1", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::cache_r_exp", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::stage2", skip_all)]
    ...
    /// now, folded into stage 3/4 instead).
    ...
    /// opening point, established by stage 2's `exp_mult_prover`, independent of `sat_diff`.
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::cache_z_hi_lo", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampProver::stage3", skip_all)]
    ...
    /// Build stage 3 sumcheck instances: exp-digit lookups (unchanged) plus the new saturating-
    /// clamp lookup's execution proof (folded directly into this batch, replacing the old
    /// `sat_diff_rc_prover`). `z_tensor` is computed by the caller (`prove`), since by this point
    ...
    /// Build stage 4 sumcheck instances: exp-digit one-hot checks (unchanged) plus the new
    /// saturating-clamp lookup's one-hot checks (replacing the old `sat_diff` triple).
...
// ---------------------------------------------------------------------------
// SoftmaxLastAxisSatClampVerifier methods
// ---------------------------------------------------------------------------

impl SoftmaxLastAxisSatClampVerifier {
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::new", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::inv_sum_evals", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::cache_exp_sum", skip_all)]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::cache_R", skip_all)]
    ...
    #[tracing::instrument(
        name = "SoftmaxLastAxisSatClampVerifier::build_stage1_verifiers",
        skip_all
    )]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::cache_r_exp", skip_all)]
    ...
    #[tracing::instrument(
        name = "SoftmaxLastAxisSatClampVerifier::build_stage2_verifiers",
        skip_all
    )]
    ...
    /// Verifier counterpart of [`SoftmaxLastAxisSatClampProver::cache_z_hi_lo`]: append the
    /// `SoftmaxZHi`/`SoftmaxZLo` opening requests (claims loaded from the proof).
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::cache_z_hi_lo", skip_all)]
    ...
    #[tracing::instrument(
        name = "SoftmaxLastAxisSatClampVerifier::build_stage3_verifiers",
        skip_all
    )]
    ...
    #[tracing::instrument(name = "SoftmaxLastAxisSatClampVerifier::operand_link", skip_all)]
    ...
    #[tracing::instrument(
        name = "SoftmaxLastAxisSatClampVerifier::build_stage4_verifiers",
        skip_all
    )]
...
mod tests {
    ...
    fn softmax_last_axis_satclamp_model(input_shape: &[usize], scale: u32) -> Model {
    ...
    fn run_softmax_scale_test(scale: u32) {
        ...
        let model = softmax_last_axis_satclamp_model(&input_shape, scale);
        unit_test_op(model, &[input]);
    }

    #[test]
    fn test_softmax_satclamp_model_scale() {
    ...
    #[test]
    fn test_softmax_satclamp_gpt2_sized() {
        ...
        let input = Tensor::new(Some(&data), &input_shape).unwrap();
        let model = softmax_last_axis_satclamp_model(&input_shape, scale as u32);
        unit_test_op(model, &[input]);
    }
}
