...
pub struct TestTable<const WORD_SIZE: usize>;

impl<const WORD_SIZE: usize> AtlasLookupTable for TestTable<WORD_SIZE> {
    fn materialize_entry(&self, index: u64) -> u64 {
        ...
        if sign_bit & index == 0 {
            ...
            (!index).wrapping_add(0) % (1 << WORD_SIZE)
        }
    }
...
impl<const WORD_SIZE: usize> PrefixSuffixDecomposition<WORD_SIZE> for TestTable<WORD_SIZE> {
    fn suffixes(&self) -> Vec<Suffixes> {
        vec![Suffixes::One, Suffixes::Relu, Suffixes::AbsIncr]
    }
...
mod test {
    ...
    #[test]
    fn prefix_suffix() {
        prefix_suffix_test::<Fr, TestTable<32>>();
    }

    #[test]
    fn mle_full_hypercube() {
        lookup_table_mle_full_hypercube_test::<Fr, TestTable<8>>();
    }

    #[test]
    fn mle_random() {
        lookup_table_mle_random_test::<Fr, TestTable<32>>();
    }
}
