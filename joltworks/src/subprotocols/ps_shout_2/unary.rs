...
pub struct ReadRafSumcheckParams<F, LUT, const LOG_K: usize>
where
    F: JoltField,
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    /// γ used for batching lookup output with operand contributions.
    pub gamma: F,
    ...
    pub rv_claim: F,
    /// Operand claim for RAF checking
    pub operand_claim: F,
}

impl<F, LUT, const LOG_K: usize> ReadRafSumcheckParams<F, LUT, LOG_K>
where
    F: JoltField,
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
...
impl<F, LUT, const LOG_K: usize> SumcheckInstanceParams<F> for ReadRafSumcheckParams<F, LUT, LOG_K>
where
    F: JoltField,
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    ...
    fn input_claim(&self, _accumulator: &dyn OpeningAccumulator<F>) -> F {
        self.rv_claim + self.gamma * self.operand_claim
    }
...
pub struct ReadRafSumcheckProver<F, LUT, const LOG_K: usize>
where
    F: JoltField,
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    /// Shared params between prover and verifier, including table info and opening claims.
    params: ReadRafSumcheckParams<F, LUT, LOG_K>,
...
impl<F, LUT, const LOG_K: usize> ReadRafSumcheckProver<F, LUT, LOG_K>
where
    F: JoltField,
    LUT: JoltLookupTable + PrefixSuffixDecompositionTrait<XLEN>,
{
    fn new_inner(
        ...
        let phases = NUM_PHASES; // TODO: optimize for 32 rounds
    ...
    pub fn new(
        ...
        drop(_guard);
        Self::new_inner(params, lookup_indices, identity_ps)
    }
    ...
    fn init_phase(&mut self, phase: usize) {
        ...
        identity_ps.init_Q(u_evals, &self.lookup_indices);
    ...
    fn prover_msg_raf(&self) -> [F; 2] {
        ...
        [
            F::from_montgomery_reduce(
                operand_0.mul_trunc::<4, 9>(self.params.gamma.as_unreduced_ref()),
            ),
            F::from_montgomery_reduce(
                operand_2.mul_trunc::<4, 9>(self.params.gamma.as_unreduced_ref()),
            ),
        ]
    }
...
impl<F: JoltField, FS: Transcript, T, const LOG_K: usize> SumcheckInstanceProver<F, FS>
    for ReadRafSumcheckProver<F, T, LOG_K>
where
