//! Constraint types for BlindFold claim binding.
//!
//! `OutputClaimConstraint` describes how a sumcheck output claim is derived
//! from polynomial openings and verifier challenges, as a sum-of-products:
//!   output = Sum_i coeff_i * Prod_j factor_{i,j}
...
/// General form: output = Sum_i coeff_i * Prod_j factor_{i,j}
///
/// Handles all constraint patterns:
/// - Linear: coeff * factor (single factor per term)
/// - Product: 1 * (a * b * c) (single term, multiple factors)
/// - Sum-of-products: alpha_1*(a*b) + alpha_2*(c*d)
/// - Differences: alpha*(a) + (-alpha)*(b)
/// - Squares: 1*(a*a)
