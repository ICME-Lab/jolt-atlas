...
///
/// ```text
/// LeftInstructionInput(x) = LeftInstructionInputIsRs1(x) * Rs1Value(x)
...
impl<F: JoltField> InstructionInputSumcheck<F> {
    ...
    pub fn new_prover(
        ...
        let prover_state = ProverState::generate(
            pp.bytecode(),
            trace,
...
impl<F: JoltField> SumcheckInstance<F> for InstructionInputSumcheck<F> {
        ...
        let [
            ...
            .map(|j_hi| {
                ...
                for j_lo in 0..out_len {
                    ...
                    let right_is_rs2_at_1_j = state.right_is_rs2_poly.get_bound_coeff(j + half_n);
                    ...
                    let right_is_imm_at_1_j = state.right_is_imm_poly.get_bound_coeff(j + half_n);
                    ...
                    let right_at_0_j =
                        right_is_rs2_at_0_j * rs2_value_at_0_j + right_is_imm_at_0_j * imm_at_0_j;
    ...
    fn expected_output_claim(
        ...
        let left_instruction_input = left_is_rs1_eval * rs1_value_eval;
        ...
        (
            ...
            .for_each(
                |(
                    ...
                    *left_is_rs1_eval = flags[CircuitFlags::LeftOperandIsTs1Value].into();
                    *right_is_rs2_eval = flags[CircuitFlags::RightOperandIsTs2Value].into();
                    *right_is_imm_eval = flags[CircuitFlags::RightOperandIsImm].into();
                    *rs1_value_eval = cycle.ts1_read();
                    *rs2_value_eval = cycle.ts2_read();
                    *imm_eval = instruction.imm;
                },
            );
...
/// and a hint `p(0) + p(1)`.
pub fn from_even_evals_and_hint<F: JoltField>(hint: F, evals: Vec<F>) -> Vec<F> {
