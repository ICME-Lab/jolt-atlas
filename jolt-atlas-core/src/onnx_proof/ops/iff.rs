...
impl<F: JoltField, T: Transcript> SumcheckInstanceProver<F, T> for IffProver<F> {
    ...
    fn cache_openings(
        ...
        let mask_opening_id = node.build_opening_id(OpeningTarget::Input(0));
        ...
        let a_opening_id = node.build_opening_id(OpeningTarget::Input(1));
        accumulator.append_virtual(
            transcript,
            a_opening_id,
            opening_point.clone(),
        ...
        let b_opening_id = node.build_opening_id(OpeningTarget::Input(2));
        accumulator.append_virtual(
            transcript,
            b_opening_id,
            opening_point,
