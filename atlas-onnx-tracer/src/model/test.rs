...
impl ModelBuilder {
    ...
    /// Uses a default fixed-point scale of 8 bits for the fractional part.
    pub fn new() -> Self {
        Self::with_scale(DEFAULT_SCALE)
    }

    /// Create a new empty model builder with a custom scale (number of fractional bits).
    ...
    pub fn identity(&mut self, input: Wire) -> Wire {
        let id = self.alloc();
        let output_dims = self.nodes[&input].raw_or_padded_output_dims();
    ...
    pub fn add(&mut self, a: Wire, b: Wire) -> Wire {
        let id = self.alloc();
        let output_dims = self.nodes[&a].raw_or_padded_output_dims();
    ...
    pub fn iff(&mut self, condition: Wire, true_branch: Wire, false_branch: Wire) -> Wire {
        let id = self.alloc();
        let output_dims = self.nodes[&true_branch].raw_or_padded_output_dims();
        let node = ComputationNode::new(
            id,
            Operator::Iff(Iff),
    ...
    pub fn moveaxis(&mut self, input: Wire, source: usize, destination: usize) -> Wire {
        let id = self.alloc();
        let input_dims = self.nodes[&input].raw_or_padded_output_dims();
    ...
    pub fn gather(
        ...
        let id = self.alloc();
        let dict_len = self.nodes[&data].raw_or_padded_output_dims()[axis];
    ...
    pub fn concat(&mut self, inputs: &[Wire], axis: isize) -> Wire {
        ...
        let id = self.alloc();
        let first_dims = self.nodes[&inputs[0]].raw_or_padded_output_dims();
