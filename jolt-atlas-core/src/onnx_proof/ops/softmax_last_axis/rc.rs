...
///
/// Both softmax range checks (remainder, exp-remainder) follow the same
/// pattern: read a claim and cycle point from one virtual polynomial, and
/// emit an `ra` polynomial for the one-hot encoding.  This struct
...
///
/// All four softmax one-hot checks (remainder, exp-remainder, exp-hi,
/// exp-lo) follow the same pattern.  This struct captures the
