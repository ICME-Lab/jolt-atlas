...
use crate::poly::commitment::pedersen::hyrax::{self, HyraxOpeningProof};
...
/// it from the proof and absorbs into transcript.
...
impl<'a, F: JoltField, C: JoltCurve<F = F>> BlindFoldProver<'a, F, C> {
    ...
    pub fn prove<T: Transcript>(
        ...
        let (t_row_commitments, t_row_blindings) =
            commit_cross_term_rows(self.gens, &cross_term, R_E, C_E, &mut rng);
        ...
        let folded_witness =
            real_witness.fold(&random_witness, &cross_term, &t_row_blindings, r_field);
        ...
        for _ in 0..num_vars {
            ...
            for coeff in &compressed.coeffs_except_linear_term {
        ...
        transcript.append_scalar(&az_r);
        transcript.append_scalar(&bz_r);
        transcript.append_scalar(&cz_r);
...
pub use super::BlindFoldVerifyError;
...
fn append_instance_to_transcript<F: JoltField, C: JoltCurve<F = F>>(
    ...
    transcript.append_scalar(&instance.u);
    for com in &instance.round_commitments {
        transcript.append_serializable(com);
    }
    for com in &instance.output_claims_row_commitments {
        transcript.append_serializable(com);
    }
    for com in &instance.noncoeff_row_commitments {
        transcript.append_serializable(com);
    }
    for com in &instance.e_row_commitments {
        transcript.append_serializable(com);
    }
    for com in &instance.eval_commitments {
        transcript.append_serializable(com);
    }
}
#[cfg(test)]
